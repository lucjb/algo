 ...
 * zero-indexed array A, returns 1 if array A is a permutation and 0 if it is
 * not. For example, given array A such that: A[0] = 4 A[1] = 1 A[2] = 3 A[3] =
 * 2 the function should return 1. Given array A such that: A[0] = 4 A[1] = 1
 * A[2] = 3 the function should return 0. Assume that: N is an integer within
 * the range [1..100,000]; each element of array A is an integer within the
 * range [1..1,000,000,000]. Complexity: expected worst-case time complexity is
 * O(N); expected worst-case space complexity is O(N), beyond input storage (not
